...
class JobDatabase:
    ...
    def insert_job(self, job: Dict) -> bool:
        ...
        try:
            ...
            ''', (
                ...
                job.get('salary'), job.get('job_type'), job.get('experience_level'),
                job['source'], job.get('scraped_at', datetime.now().isoformat()),
                json.dumps(job)
            ))
            self.conn.commit()
    ...
    def update_match_score(self, job_id: str, match_result: Dict) -> bool:
        ...
        try:
            ...
            if match_result['match_score'] >= 0.80:
                self.log_activity(job_id, 'high_match', 
    ...
    def update_status(self, job_id: str, status: str, notes: str = None) -> bool:
    ...
    def log_activity(self, job_id: str, action: str, details: str):
        """Log activity"""
        try:
            cursor = self.conn.cursor()
            cursor.execute('''
