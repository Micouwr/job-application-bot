...
class JobDatabase:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        try:
            with self.get_session() as session:
                ...
                stats["recent_activity"] = [log.to_dict() for log in recent_logs]
    ...
    def get_job_by_id(self, job_id: str) -> Optional[Dict[str, Any]]:
        """Fetch a single job by its ID with all related details."""
        try:
            with self.get_session() as session:
                ...
                if job.match_details:
                    job_data.update(job.match_details.to_dict())

                if job.applications:
                    # Assuming one application per job for simplicity
                    job_data.update(job.applications[0].to_dict())
    ...
    def update_job_status(self, job_id: str, new_status: str) -> bool:
        """Update the status of a job and log the activity."""
        try:
            with self.get_session() as session:
                job = session.query(Job).filter_by(id=job_id, is_deleted=False).first()
                if not job:
                ...
                session.add(job)  # Explicitly add the object to the session to track changes
                
                self._log_activity(session, job_id, "status_change", f"Status changed from '{old_status}' to '{new_status}'")
