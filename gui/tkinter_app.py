...
"""
    ...
    def _create_custom_prompt_tab(self):
        ...
        try:
            ...
            default_template = "# Custom prompt template\n# Use variables: role_level, company_name, job_title, job_description, resume_text\n\nWrite your custom prompt instructions here...\n"
    ...
    def load_custom_prompt(self):
        """Load existing custom prompt template."""
        prompt_name = self.prompt_name_entry.get().strip()
        if not prompt_name.endswith('.txt.j2'):
            prompt_name += '.txt.j2'
        
        try:
            with open(f"prompts/user/{prompt_name}", 'r') as f:
                content = f.read()
            self.prompt_text.delete('1.0', tk.END)
            self.prompt_text.insert("1.0", content)
            messagebox.showinfo("Success", f"Loaded {prompt_name}")
        except Exception as e:
...
if __name__ == "__main__":
    main()
ENDOFFILEcat > gui/tkinter_app.py << 'ENDOFFILE'
import tkinter as tk
from tkinter import ttk, messagebox, filedialog, scrolledtext
import json
import os
