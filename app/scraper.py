...
class JobScraper:
    ...
    def _guess_job_title_from_text(self, job_text: str) -> str:
        """
        A simple utility to guess the job title from the first non-empty line
        of a job description text block for logging purposes.
        """
        lines = job_text.split('\n')
        for line in lines:
            title = line.strip()
            if title:
                # Limit title length for cleaner log/DB entries
                return title[:100] 
        return "Untitled Job Listing"
